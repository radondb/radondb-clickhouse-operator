...
RELEASE_VERSION=$(get_file https://raw.githubusercontent.com/RadonDB/radondb-clickhouse-operator/master/release)
OPERATOR_VERSION="${OPERATOR_VERSION:-$RELEASE_VERSION}"
OPERATOR_IMAGE="${OPERATOR_IMAGE:-radondb/chronus-operator:$OPERATOR_VERSION}"
METRICS_EXPORTER_IMAGE="${METRICS_EXPORTER_IMAGE:-radondb/chronus-metrics-operator:$OPERATOR_VERSION}"
...
kubectl apply --namespace="${OPERATOR_NAMESPACE}" -f <( \
    get_file https://raw.githubusercontent.com/RadonDB/radondb-clickhouse-operator/master/deploy/operator/clickhouse-operator-install-template.yaml | \
